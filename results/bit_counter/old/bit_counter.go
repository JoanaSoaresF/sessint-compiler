...
func bit(b int) func(_x *_state_0, d *_state_0) {
	return func(c *_state_0, d *_state_0) {
		...
		case "inc":
			...
			if b == 0 {
				c_ := init_state_0(make(chan interface{}))
				go bit(1)(c_, d)
				// FWD c c_ Start
				for {
					c_c0 := c0.Recv()
					...
					switch c_c0 {
					case "val":
						c_0 := c_.ls["val"].(*_state_1)
						c1 := c0.ls["val"].(*_state_1)
						c1c_0, c1_c_0 := c_0.Recv()
						c_ = c1_c_0
						c0 = c1.Send(c1c_0)
					case "halt":
						c_0 := c_.ls["halt"].(*_state_2)
						c1 := c0.ls["halt"].(*_state_2)
						c_0.Recv()
						c1.Send(nil)
						return
					}
		...
		case "val":
			...
			for {
				...
				case "val":
					c_0 := c_.ls["val"].(*_state_1)
					c2 := c1.ls["val"].(*_state_1)
					c2c_0, c2_c_0 := c_0.Recv()
					c_ = c2_c_0
					c1 = c2.Send(c2c_0)
				case "halt":
					c_0 := c_.ls["halt"].(*_state_2)
					c2 := c1.ls["halt"].(*_state_2)
					c_0.Recv()
					c2.Send(nil)
					return
				}
