...
type msg = MsgLabel of var | MsgExp of exp | MsgChan of msg Event.channel
...
let[@warning "-8"] eval_uop op =
  match op with Neg -> fun (Num n) -> Num (-n) | Not -> fun (Bool b) -> Bool (not b)

(* Replace x with value of e1 in expression e2 *)
let rec subst x e1 e2 =
  match e2 with
  | UnitVal | Num _ | Bool _ -> e2
  | Var y -> if x = y then e1 else e2
  | FunDef (y, opt, e, ret_ty) ->
      if x == y then e2 else FunDef (y, opt, subst x e1 e, ret_ty)
  | RecFunDef (y, opt, e, ret_ty) ->
      if x == y then e2 else FunDef (y, opt, subst x e1 e, ret_ty)
  | BOp (bop, e1', e2') -> BOp (bop, subst x e1 e1', subst x e1 e2')
  | UOp (uop, e) -> UOp (uop, subst x e1 e)
  | Let (y, e1', e2') ->
      if x != y then Let (y, subst x e1 e1', subst x e1 e2')
      else Let (y, subst x e1 e1', e2')
...
and subst_proc x e p =
  ...
  | TailSpawn (_, _, _, _, _, _, _) -> assert false (*does not occur in this phase*)
  ...
  | MultiSend (_, _, _,  _) | MultiRecv (_, _,  _, _) ->
      assert false (*does not occur in this phase*)
...
let rec eval env e =
  match e with
  | UnitVal | Num _ | Bool _ | FunDef (_, _, _, _) -> e
  ...
  | Let (x, e1, e2) ->
      ...
      eval (StrMap.add x e1' env) (subst x e1' e2)
  | FunApp (e1, e2) -> (
  ...
  | ExecExp exp -> (
      ...
      | ProcExp (cname, proc, _, _) ->
          exec StrMap.empty env StrMap.empty proc cname (Event.new_channel ());
          e
      | _ -> assert false (* should never happen if well typed *))
  | RecFunDef (_, _, _, _) -> e
              ...
              let unfold_proc =
                Recv
                  ( illegal_id,
                    d,
                    Some t,
                    Send (c', Var illegal_id, Some t, Fwd (Some st', c', d)) )
              in
              exec lin_ctxt env rec_env unfold_proc cname
                channel (* recv in x, send in y *)
          | STRecv (t, st') ->
              let illegal_id = "%" in
              let unfold_proc =
                Recv
                  ( illegal_id,
                    c',
                    Some t,
                    Send (d, Var illegal_id, Some t, Fwd (Some st', c', d)) )
              in
              exec lin_ctxt env rec_env unfold_proc cname
                channel (* recv in y, send in x *)
          | STEnd ->
              let unfold_proc = Wait (d, Close c') in
              exec lin_ctxt env rec_env unfold_proc cname channel (* wait on x, close y *)
          | STExtChoice l ->
              let fold_fun new_pair_list (label, label_type) =
                ( label,
                  ( Label (d, label, Fwd (Some label_type, c', d), Some label_type),
                    Some label_type ) )
                :: new_pair_list
              in
              let unfold_proc = Choice (c', List.fold_left fold_fun [] l) in
              exec lin_ctxt env rec_env unfold_proc cname
                channel (* choice on y, label on x *)
          | STIntChoice l ->
              let fold_fun new_pair_list (label, label_type) =
                ( label,
                  ( Label (c', label, Fwd (Some label_type, c', d), Some label_type),
                    Some label_type ) )
                :: new_pair_list
              in
              let unfold_proc = Choice (d, List.fold_left fold_fun [] l) in
              exec lin_ctxt env rec_env unfold_proc cname
                channel (* choice on x, label on y*)
          | STSendChan (sent_st, st') ->
              let illegal_id = "%" in
              let unfold_proc =
                RecvChan
                  ( illegal_id,
                    d,
                    Some sent_st,
                    SendChan (c', illegal_id, Some sent_st, Fwd (Some st', c', d)) )
              in
              exec lin_ctxt env rec_env unfold_proc cname
                channel (* recv in x, send in y *)
          | STRecvChan (recvd_st, st') ->
              let illegal_id = "%" in
              let unfold_proc =
                RecvChan
                  ( illegal_id,
                    c',
                    Some recvd_st,
                    SendChan (d, illegal_id, Some recvd_st, Fwd (Some st', c', d)) )
              in
              exec lin_ctxt env rec_env unfold_proc cname
                channel (* recv in y, send in x *)
          | STRec (x, st') ->
              let new_rec_env = StrMap.add x st' rec_env in
              exec lin_ctxt env new_rec_env (Fwd (Some st', c', d)) cname channel
          | STVar x ->
              let st' = StrMap.find x rec_env in
              exec lin_ctxt env rec_env (Fwd (Some st', c', d)) cname channel
          | STUVar _ -> assert false
          | STMultiSend _ | STMultiRecv _ ->
              assert false (* should  not happen in this phase *))
  ...
  *)
  ...
  | Spawn (c', e, _, p, _) -> (
      ...
      | ProcExp (cname', p', _, _) ->
          let _ =
            Thread.create
              (fun _ -> exec StrMap.empty env StrMap.empty p' cname' new_chan)
              ()
          in
  ...
  | Choice (c', ls) -> (
      if cname = c' then
        ...
        | MsgLabel label -> (
            ...
            | Some (p, _) ->
                exec lin_ctxt env rec_env p cname channel
                (* proceed as the process that matches the received label *)
            | None -> assert false (* should never happen if well typed *))
        | _ -> assert false (* should never happen if well typed *)
      else
        (* have an internal choice (sum type) in ctxt *)
        let chan = StrMap.find c' lin_ctxt in
  ...
  | Label (c', l, p, _) ->
      if cname = c' then
        ...
        exec lin_ctxt env rec_env p cname channel
  | Print (e, p) ->
      print_endline @@ string_from_exp (eval env e);
      exec lin_ctxt env rec_env p cname channel
  | If (e, p1, p2) -> (
      match eval env e with
      | Bool b ->
          if b then exec lin_ctxt env rec_env p1 cname channel
          else exec lin_ctxt env rec_env p2 cname channel
      | _ -> assert false (* should never happen if well typed *))
  | MultiSend (_, _,  _, _) | MultiRecv (_, _, _, _) ->
      assert false (* does not occur in this phase*)

let eval_decl env decl =
  match decl with Decl (x, _, exp) -> (x, eval (StrMap.add x exp env) exp)
...
let eval_program prog =
  ...
  | Prog (ldecls, e) ->
      let env =
        List.fold_left
          (fun acc decl ->
            let key, data = eval_decl acc decl in
            StrMap.add key data acc)
          StrMap.empty ldecls
      in
      eval env e
